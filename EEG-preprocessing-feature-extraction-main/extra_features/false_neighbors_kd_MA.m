function [sig_out,sig_out_chan] = false_neighbors_kd_MA(eeg)
...
for j=1:channels
    ...
    tt = ami(x,npts,max_delay);
    [~,idx] = min(tt);
    nn = false_neighbors_kd(x, idx, maxdims, npts, 1, 0, 0, 0);
    mindim = find((mean(nn.z./nn.d > distance_thresh) < 01.) == 1);
    %sept10: handle empty case
    if isempty(mindim)
        out = 0;
    else
        out = mindim(1);
