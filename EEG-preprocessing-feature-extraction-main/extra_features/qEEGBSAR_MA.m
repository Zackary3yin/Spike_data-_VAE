function  [sig_out, sig_out_chan]=qEEGBSAR_MA(eeg, fs) %(samples x ch)
    ...
    if minchan == 19
        ...
        for channel=1:size(EEG,2)
            ...
            eeg_chan(channel).BSAR = BSAR_chan(channel);
            eeg_chan(channel).channel =channel;
        end  
...
%         BSAR=mean(BSAR_chan);
    ...
    sig_out_chan = [eeg_chan.BSAR];
    sig_out = mean([eeg_chan.BSAR]);
end